    ...
    <x-card class="space-y-8">
        ...
        <div>
            @if (!(auth()->user()->is_verified) && auth()->user()->role != 'admin')
                <a href="{{route('auth.verify.email')}}" class="border rounded-md px-2 py-2 text-sm border-red-400 bg-red-100 text-red-400 hover:bg-red-200 hover:text-red-500 hover:border-red-500" >Verify Your Email</a>
            @endif
        </div>
